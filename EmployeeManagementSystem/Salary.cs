...
");
                ...
                if(check == DialogResult.Yes)
                {
                    if(connect.State == ConnectionState.Closed)
                    {
                        try
                        {
                            ...
                            string updateData = "UPDATE employees SET salary ='"+salary_salary.Text+"'" +
                                ", update_date = @updateData WHERE Emp_Id = '"+salary_employeeID.Text+"'";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            ds = new DataSet();
            da.Fill(ds);
            string xml = @"C:/EmployeeManagementSystem/EmployeeManagementSystem/data.xml";
            ds.WriteXmlSchema(xml);

            Crypath = @"C:/EmployeeManagementSystem/EmployeeManagementSystem/salary.rpt";
            cr.Load(Crypath);
            cr.SetDataSource(ds);
